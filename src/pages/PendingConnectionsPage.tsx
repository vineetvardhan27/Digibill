  ...
  const fetchRequests = async () => {
    try {
      ...
      const [receivedRes, sentRes] = await Promise.all([
        axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/connections/pending`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        }),
        axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/connections?status=pending`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        })
      ]);
      ...
      if (sentRes.data.success) {
        // filter for initiatedBy shop
        const sent = sentRes.data.data.filter((r: ConnectionRequest) => r.initiatedBy === 'shop');
  ...
  const handleRespond = async (id: string, action: 'accept' | 'reject') => {
    try {
      const response = await axios.patch(
        `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/connections/${id}/respond`,
        { action },
        { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
      );
      ...
      if (response.data.success) {
        toast({
          ...
          description: action === 'accept' 
            ? 'You are now connected and can manage bills.' 
        ...
        <TabsContent value="received">
          {isLoading ? renderSkeleton() : receivedRequests.length === 0 ? (
            <div className="text-center py-12 border rounded-lg bg-background">
              <p className="text-muted-foreground">You have no pending connection requests from suppliers.</p>
            </div>
          ) : (
            <div className="space-y-4">
                ...
                <Card key={req._id}>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-lg">
                      {req.supplierAccountId?.businessName || 'Unknown Supplier'}
                    </CardTitle>
                    <p className="text-sm text-muted-foreground">
                      {req.supplierAccountId?.location?.city || 'Unknown Location'}
                    </p>
                  </CardHeader>
                  <CardContent>
                    {req.requestNote ? (
                      <div className="bg-muted p-3 rounded-md text-sm border">
                        <p className="font-semibold mb-1">Note from supplier:</p>
                        <p>"{req.requestNote}"</p>
                      </div>
                    ) : (
